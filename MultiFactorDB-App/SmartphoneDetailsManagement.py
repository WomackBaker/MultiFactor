...
class SmartphoneDetailsManagement:
    ...
    def layout(self):
        ...
        return [
            ...
            [sg.Table(values=[], headings=['DetailID', 'Latitude', 'Longitude', 'ipString', 'AvailableMemory', 'Battery'], key='-DETAILS_TABLE-', enable_events=True, select_mode=sg.TABLE_SELECT_MODE_EXTENDED)],
            buttons,
            [sg.Button('Back')]
        ]
    ...
    def fetch_smartphone_details(self):
        ...
        query = "SELECT DetailID, Latitude, Longitude, ipString, AvailableMemory, Battery FROM SmartPhoneDetails WHERE DeviceID = %s"
    ...
    def add_smartphone_details(self, details_data):
        ...
        query = "INSERT INTO SmartPhoneDetails (DeviceID, Latitude, Longitude, ipString, AvailableMemory, Battery) VALUES (%s, %s, %s, %s, %s, %s)"
    ...
    def edit_smartphone_details(self, detail_id, details_data):
        connection = self.db_connection.create_connection()
        cursor = connection.cursor()
        query = "UPDATE SmartPhoneDetails SET Latitude=%s, Longitude=%s, ipString=%s, AvailableMemory=%s, Battery=%s WHERE DetailID=%s"
    ...
    def open_edit_details_window(self, details_data):
        layout = [
            ...
            [sg.Text('Current IP'), sg.Input(details_data['ipString'], key='-CURRENT_IP-')],
