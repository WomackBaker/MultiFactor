...
class MainApp:
    ...
    def smartphone_details_management_window(self, device_id, role):
        ...
        window['-DETAILS_TABLE-'].update(values=[[d['DetailID'], d['Latitude'], d['Longitude'], d['ipString'], d['AvailableMemory'], d['Battery']] for d in details])
            ...
            if event == 'Add Smartphone Details':
                ...
                if selected_rows:
                    ...
                    details_data = {
                        ...
                        'ipString': details[selected_rows[0]]['ipString'],
                ...
                if selected_rows:
                    ...
                    details = smartphone_details_management.fetch_smartphone_details()
                    window['-DETAILS_TABLE-'].update(values=[[['DetailID'], d['Latitude'], d['Longitude'], d['ipString'], d['AvailableMemory'], d['Battery']] for d in details])
            elif event == 'Delete Smartphone Details' and role == 'Admin':
                selected_rows = values['-DETAILS_TABLE-']
                if selected_rows:
                    detail_ids = [details[row]['DetailID'] for row in selected_rows]
