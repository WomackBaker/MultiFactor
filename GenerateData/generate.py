#!/usr/bin/env python3
"""
realistic_single_user.py
-------------------------------------------------
Generate 10 000 chronological, human-like mobile-session records
for ONE US user.  All columns remain numeric for ML pipelines.
"""
...
CLICK_CODES      = [1,2,3]
IP_REP_CODES     = [1,2,3]
DEVICE_TYPES     = [0,1]                 # 0=phone, 1=tablet
...
def rand_ip(pool: tuple[int,int]|None=None) -> int:
    first_pool = [24,66,67,68,69,72,74,99,130,140,142,144,147,152,155,
                  157,160,168,170,192,199,204,205,206,207,208,209,216]
    if pool:          # stable ISP /16
        a,b = pool
    else:             # random US-ish /24 (VPN)
        a,b = random.choice(first_pool), random.randint(0,255)
    c,d = random.randint(0,255), random.randint(0,255)
...
# ─────────────── DATA GENERATOR ────────────────
def generate_rows(p:dict, n:int=10_000) -> list[list]:
    ...
    for day_idx in range(366):
        ...
        sessions = min(sessions, n-len(rows))            # don’t overrun target

        sh_start, sh_end = SHIFT_PROFILES[p["shift_code"]]
        span_minutes     = (sh_end - sh_start)*60

        # *** FIX: choose only offsets that leave ≥5 min in shift ***
        ...
        for off in sess_offsets:
            ...
            # location cluster
            if random.random()<0.05:        # 5 % roaming (50-300 km)
                lat, lon = bearing_offset(p["home_lat"], p["home_lon"],
                                          random.uniform(50,300),
                                          random.random()*2*math.pi)
            else:
                if off < span_minutes/2: lat, lon = p["work_lat"], p["work_lon"]
                else:                     lat, lon = p["home_lat"], p["home_lon"]
...
def write_csv(name:str="realistic_single_user_10k.csv", rows:int=10_000):
    prof = make_profile()
    data = generate_rows(prof, rows)
    with open(name,"w",newline="",encoding="utf-8") as f:
        csv.writer(f).writerows([HEADERS, *data])
    print(f"[+] {rows:,} rows written → {name}")
